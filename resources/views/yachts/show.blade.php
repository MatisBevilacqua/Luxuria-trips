...
<div class="pt-[15vh] pl-[5vh] pr-[5vh]">
    <div class="text-center mb-8">
        <h1 class="text-4xl font-bold text-gray-900">{{ $yacht->name }}</h1>
        <p class="text-lg text-gray-600 mt-2">Explore the luxurious features of this exquisite yacht.</p>
    </div>

    <div class="bg-white shadow-lg rounded-lg overflow-hidden">
        <div class="relative">
            @if($yacht->images->isNotEmpty())
                <img src="{{ $yacht->images->first()->url }}" alt="Yacht Image" class="w-full h-80 object-cover">
            @else
        ...
        <div class="p-6">
            <div class="text-gray-800">
                <h2 class="text-2xl font-semibold mb-4">About This Yacht</h2>
                <p class="text-base mb-4">{{ $yacht->description }}</p>
            </div>

            <div class="bg-gray-100 p-4 rounded-lg mb-4">
                <div class="flex justify-between mb-2">
                    <span class="font-semibold text-gray-700">Price per Hour:</span>
                    <span class="text-xl font-bold text-gray-900">${{ $yacht->price_per_hour }}</span>
                </div>
                <div class="flex justify-between mb-2">
                    <span class="font-semibold text-gray-700">Location:</span>
                    <span class="text-gray-800">{{ $yacht->location }}, {{ $yacht->city }}</span>
                </div>
            </div>
        </div>
                    ...
                    @foreach($yacht->images as $index => $image)
                        @if ($index > 0)
                            <div class="w-1/2 md:w-1/3 lg:w-1/4">
                                <img src="{{ $image->url }}" alt="Additional Yacht Image" class="w-full h-40 object-cover rounded-lg shadow-sm">
                            </div>
                        @endif
                    @endforeach
                </div>
    ...
    <div class="mt-8 p-6 bg-blue-50 rounded-lg shadow-md">
        <h2 class="text-2xl font-semibold text-gray-900 mb-4">How to Reserve This Yacht</h2>
        <p class="text-base text-gray-700 mb-4">
            Ready to book this luxurious yacht? Follow the steps below to make your reservation:
        </p>
        <div class="bg-white p-4 rounded-lg shadow-sm mb-4">
            ...
            <p class="text-base text-gray-700 mb-2 font-semibold">Subject: {{ $yacht->name }} Reservation Request</p>
        </div>
        <div class="bg-white p-4 rounded-lg shadow-sm mb-4">
